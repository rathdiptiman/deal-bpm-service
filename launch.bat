...
if "%*" == "" (
	...
	for %%s in ("-kjar" "deal-bpm-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name deal-bpm-service apps/deal-bpm-service:1.0.0
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app deal-bpm-service:1.0.0
		call oc expose svc/deal-bpm-service
	) else (
