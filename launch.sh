...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=("*-kjar" "deal-bpm-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name deal-bpm-service apps/deal-bpm-service:1.0.0
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app deal-bpm-service:1.0.0
    oc expose svc/deal-bpm-service
else

	echo "Launching the application locally..."
	pattern="deal-bpm-service"
	files=( $pattern )
	cd ${files[0]}
